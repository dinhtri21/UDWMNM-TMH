			...
			@if($products->sale == 0)
				...
				<div class="product_info col-md-6">
					...
					<div class="product_id">
						<p>
						<h4>Product ID: </h4> {{$products->msp}} </p>
					</div>
					<div class="product_price">
						<h4>
							<p> Price: </p>
						</h4>{{ number_format($products->product_price, 0, ',', '.') }} VND
					</div>
					<form method="POST" action="#">
						<input type="hidden" value="{{$products->id}}">
						<input type="hidden" value="{{ csrf_token() }}">
						<a href="#"><button type="button" class="btn btn-default btn-lg btn-block btn-add"
								style="width:300px;" align=center>Add To Cart</button></a></p>
					</form>
			...
			@else
				...
				<div class="product_info col-md-6">
					...
					<div class="product_price">
						...
						<p class="price"><del>{{number_format($products->price, 0, ',', '.')}} VNĐ</del> ->
							{{number_format($products->product_price, 0, ',', '.')}} VNĐ
						</p>
					</div>
					<form method="GET" action="{{ route('get.add.product', $products->id) }}">
						<button type="submit" class="btn btn-default btn-lg btn-block btn-add" style="width:300px;"
							align="center">
							Add To Cart
						</button>
					</form>
